...
/*   Updated: 2020/06/16 04:46:38 by amalsago         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		cw_vm_op_xor_dir(t_cw_vm *vm, int *pos)
{
	int			arg_val;
	int			arg_pos;
...
static int		cw_vm_op_xor_ind(t_cw_vm *vm, int *pos)
{
	int			arg_val;
	int			idx_address;
	int16_t		arg_pos;
...
static int		cw_vm_op_xor_helper(t_cw_vm *vm, int *arg_val, int *pos)
{
	int			i;
	int			index;
	...
	while (++i < 2)
		if (vm->inst.types[i] == T_REG)
		{
			...
			arg_val[i] = cw_vm_op_xor_dir(vm, pos);
		else if (vm->inst.types[i] == T_IND)
			arg_val[i] = cw_vm_op_xor_ind(vm, pos);
	return (CW_SUCCESS);
}

static void		cw_vm_op_xor_body(t_cw_vm *vm)
{
	int			pos;
	int			index;
	int			reg_value;
	...
	pos = 2;
	if (cw_vm_op_xor_helper(vm, arg_val, &pos) == CW_SUCCESS)
...
void			cw_vm_op_xor(t_cw_vm *vm)
{
	if (vm->inst.args_count >= 3 && vm->inst.types[2] == T_REG)
		cw_vm_op_xor_body(vm);
	vm->game.procs->pos = (vm->game.procs->pos + 2
			+ cw_vm_add_pos(&vm->inst, 3, CW_DIR_SIZE_XOR)) % CW_MEM_SIZE;
}
