...
int		cw_vm_op_xor_dir(t_cw_battle *battle, t_cw_vm *vm, int pos)
{
	int i;
	int pow;
	int total;
...
int16_t	cw_vm_op_xor_ind(t_cw_battle *battle, t_cw_vm *vm, int pos)
{
	int				i;
	int16_t			arg;
	int				total;
	...
	i = -1;
	arg = 0;
	multiplier = ft_pow(256, CW_DIR_SIZE_XOR - 1);
	while (++i < CW_DIR_SIZE_XOR)
	{
...
void	cw_vm_op_xor_body(t_cw_inst *inst, t_cw_battle *battle, t_cw_vm *vm)
{
	int i;
	int	pos;
	int reg_value;
	...
	while (++i < 2)
		...
		{
			if (cw_vm_is_reg(vm->arena[battle->procs->pos + pos + i]) == false)
				ft_printf("ERROR\n");
			arg[i] = battle->procs->regs[(vm->arena[battle->procs->pos + pos + i]) - 1];
			pos++;
		}
		else if (inst->args[i] == T_DIR)
		{
			arg[i] = cw_vm_op_xor_dir(battle, vm, pos + 1);
			pos += CW_DIR_SIZE_XOR;
		}
		else if (inst->args[i] == T_IND)
		{
			arg[i] = cw_vm_op_xor_ind(battle, vm, pos);
			pos += 2;
		}
	reg_value = arg[0] ^ arg[1];
	battle->procs->regs[battle->procs->pos + pos - 1] = reg_value;
	battle->procs->carry = (reg_value == 0) ? 1 : 0;
	battle->procs->pos = pos + 1;
}
...
void	cw_vm_op_xor(t_cw_inst *inst, t_cw_battle *battle, t_cw_vm *vm)
{
	if (inst->args_count != 3 || inst->types[2] != T_REG)	
		ft_printf("ERROR\n");
	cw_vm_op_xor_body(inst, battle, vm);
}
