...
void	cw_vm_op_sti_body(t_cw_inst *inst, t_cw_battle *battle, t_cw_vm *vm)
{
	...
	idx_address = battle->processus->position + (arg[1] + arg[2]) % CW_IDX_MOD;
	pow = ft_pow(256, 3);
	i = -1;
	reg_value = battle->processus->registries[battle->processus->position + 1];
	while (++i < 4)
	{
