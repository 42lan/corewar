...
/*   Updated: 2020/06/18 10:31:09 by amalsago         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		cw_vm_or_dir(t_cw_vm *vm, int *pos)
{
	int			arg_val;
	int			arg_pos;
...
static int		cw_vm_or_ind(t_cw_vm *vm, int *pos)
{
	int			arg_val;
	int			idx_address;
	int16_t		arg_pos;
...
static int		cw_vm_or_helper(t_cw_vm *vm, int *arg_val, int *pos)
{
	int			i;
	int			index;
	...
	while (++i < 2)
		if (vm->inst.types[i] == T_REG)
		{
			...
			arg_val[i] = cw_vm_or_dir(vm, pos);
		else if (vm->inst.types[i] == T_IND)
			arg_val[i] = cw_vm_or_ind(vm, pos);
	return (CW_SUCCESS);
}

static void		cw_vm_or_body(t_cw_vm *vm)
{
	int			pos;
	int			index;
	int			reg_value;
	...
	pos = 2;
	if (cw_vm_or_helper(vm, arg_val, &pos) == CW_SUCCESS)
